...
1. Create an IoT Hub in the `fruit-quality-detector` resource group. Give it a unique name based around `fruit-quality-detector`.
   **NOTE: If you are using a free account, Azure will not let you create more than one IoT Hub. Skip this step and use the IoT hub you used for the soil moisture sensor labs for step 2 and step 3.** 
...
![Containers are built then pushed to a container registry, then deployed from the container registry to an edge device using IoT Edge](./img/container-edge-flow.png)
...
![THe Azure Container Registry logo](./img/azure-container-registry-logo.png)
