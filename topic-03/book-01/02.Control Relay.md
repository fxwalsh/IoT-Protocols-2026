...
![A grove relay](./img/grove-relay.png)
...
![The grove relay connected to the D5 socket, and the soil moisture sensor connected to the A0 socket](./img/pi-relay-and-soil-moisture-sensor.png)
...
1. Run the Python app. The relay will turn on and off every 10 seconds, with a half second delay between turning on and off. You will hear the relay click on then click off. An LED on the Grove board will light when the relay is on, then go out when the relay is off.
    ...
    ![The relay turning on and off](./img/relay-turn-on-off.gif)
...
1. Delete the 3 lines of code that you added to test the relay. Replace them with the following code:
    ...
    This code checks the soil moisture level from the soil moisture sensor. If it is below 200, it turns on the relay, and turns it off when it goes above 200.

    > 💁 Remember the resistive soil moisture sensor, the greater the soil moisture value, the more moisture there is in the soil and vice versa.
