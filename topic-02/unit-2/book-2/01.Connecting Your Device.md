...
1. Open the *sensor* folder in VS Code(using Remote SSH) you created in the [Raspberry Pi](https://tutors.dev/lab/setu-iot-protocols-2026/topic-01/unit-1/book-1/Measure%20Temperature) lab.

1. Activate the python virtual environment and run the command to install the MQTT pip package:
...
1. We are going to start with the simple program, `app.py`,  you created in the 1st lab (Raspberry pi) to measure temperature. It should be in the `sensors` folder on your RPi. Here is is in case you cannot find it:
    ~~~python
    import time
