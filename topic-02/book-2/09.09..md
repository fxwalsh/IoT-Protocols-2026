...
pip (**P**ackage **I**nstaller for **P**ython) is a package manager for Python that allows you to very easily install extra libraries and modules for you to use within your Python scripts. 

+ Install pip using the following command on the command line in the RPi:
  ~~~pyhton
  ~~~

  

- Python libraries, such as `requests`, are installed using `pip`. ``pip` is the package installer for Python. Use the following command on the command line on the RPi to install requests
...
response = requests.get(url) # Make HTTP request using URL
