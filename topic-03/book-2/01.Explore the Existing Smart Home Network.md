...
+ he bottom left corner of the Packet Tracer window, locate and click the **[End Devices]** icon in the top row, and the **[Home]** icon in the bottom row of the Device-Type Selection box.
...
Smart device commonly can be controlled using Phone Apps/Web applications running in the browser. And example would be **Google Home** or **Ring**. 

Applications like this have a remote, server that can mediate between the user/controlling devices and the IoT devices. This is how you can have an app that can turn your immersion off from anywhere in the world. We can simulate a similar architecture by setting up a Remote IoT Server in our Packet Tracer simulation. 

+ In the Physical View, select the  **Logical View** and click **WebServe**r in the workspace to open the configuration window. Click the **Services tab** and then click the **IoT service** in the left pane. Click the **“On”** button to enable the service.
...
+ Click the **Config tab**. Select **Global->Settings** and, for the IoT Server, select Remote Server. Enter the **IP address of the Remote IoT Server** from  use the IoT account Username and Password created in the last step. Next, click the Connect button.
