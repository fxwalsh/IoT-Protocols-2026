...
In the last step, you learned how to connect your plant soil moisture monitoring and relay control to a cloud-based IoT service. The next step is to move the server code that controls the timing of the relay to the cloud. In this lesson you will learn how to do this using serverless functions.
...
![image-20250302214103571](./img/image-20250302214103571.png)
...
![image-20250302214125014](./img/image-20250302214125014.png)
...
[![x](https://img.youtube.com/vi/8-jz5f_JyEQ/0.jpg)](https://www.youtube.com/watch?v=8-jz5f_JyEQ)
...
1. The `local.settings.json` file has a setting for the storage account that the Functions app will use. Make sure it is set thi to the following:
...
1. In  VS Code , open  `function_app.py.py` -  and add the following code at the end of the file(just under ``app = func.FunctionApp()``:


```python
    @app.function_name(name="iot-hub-trigger")
    @app.event_hub_message_trigger(arg_name="event", event_hub_name="", connection="IOT_HUB_CONNECTION_STRING", consumer_group="$Default")
...
The decorators (@app.function_name/@app.event_hub_message_trigger) contain configuration for the trigger. 

- `@app.function_name(name="iot-hub-trigger")`: This decorator registers the function with the name "iot-hub-trigger". This name is used to identify the function within the Azure Functions runtime.

- `@app.event_hub_message_trigger(arg_name="event", event_hub_name="", connection="IOT_HUB_CONNECTION_STRING", consumer_group="$Default")`: This decorator specifies that the function should be triggered by messages from an Azure Event Hub. The parameters are:
  - `arg_name="event"`: The name of the parameter that will receive the Event Hub message.
  - `event_hub_name=""`: The name of the Event Hub. You need to provide the actual name of your Event Hub here.
  - `connection="IOT_HUB_CONNECTION_STRING"`: The name of the environment variable that contains the connection string for the Event Hub.
  - `consumer_group="$Default"`: The consumer group to use when reading messages from the Event Hub.

The  configuration is in a section called `bindings`. A binding is the term for a connection between Azure Functions and other Azure services. This function has an input binding to an event hub - it connects to an event hub and receives data.
...
1. Run the following command to create a Function App:
    ...
    ```sh
    az functionapp create --resource-group soil-moisture-sensor --runtime python --functions-version 4 --os-type Linux --consumption-plan-location <location> --storage-account <storage_name> --name <functions_app_name>
    ```
    ...
    The `--functions-version 4` option sets the version of Azure Functions to use. Version 3 is the latest version.
