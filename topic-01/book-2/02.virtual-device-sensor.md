# Build a nightlight - Sensor

In this part, you will add a light sensor to your virtual IoT device.
...
1. Make sure the CounterFit web app is running from the previous lab. If not, start it.
...
1. Create a light sensor:
    ...
    ![The light sensor settings](./img/counterfit-create-light-sensor.png)

    The light sensor will be created and appear in the sensors list.

    ![The light sensor created](./img/counterfit-light-sensor.png)
...
1. In the same VS project as the previous step,  create a new files called  `app2.py` file
...
1. Add the following code to the bottom of the file to create instances of classes that manage the light sensor:
    ...
    The line `sensor = GroveLightSensor(0)` creates an instance of the `GroveLightSensor` class connecting to pin **0** - the CounterFit Grove pin that the light sensor is connected to.
...
1. From a new VS Code Terminal, run the following to run your Python app:
